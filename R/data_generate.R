...
#'
#' @return
#' A data frame of the synthetic data. Outcome is labelled as Y, exposure as w, and
#' covariates cf1-6.
#' @export
#'
#' @examples
data_generate<-function(sample_size=1000,outcome_sd = 10,gps_spec= 1, cova_spec=1){
  ...
  cf<-MASS::mvrnorm(n=size,mu=rep(0,4),Sigma=diag(4))
  ...
  if (gps_spec==1){
    ...
    treat <- 49*exp((-0.8+0.1*cf[,1]+0.1*cf[,2]- 0.1*cf[,3] + 0.2*cf[,4] + 0.1*cf5 + 0.1*cf6))/(1+exp((-0.8+0.1*cf[,1]+0.1*cf[,2]- 0.1*cf[,3] + 0.2*cf[,4] + 0.1*cf5 + 0.1*cf6)))-6 + stats::rnorm(size,sd=5)
  }else if (gps_spec==5){
    treat <- 42/(1+exp((-0.8+0.1*cf[,1]+0.1*cf[,2]- 0.1*cf[,3] + 0.2*cf[,4] + 0.1*cf5 + 0.1*cf6))) -18 + stats::rnorm(size,sd=5)
  }else if (gps_spec==6){
    treat <- log(abs(-0.8+0.1*cf[,1]+0.1*cf[,2]- 0.1*cf[,3] + 0.2*cf[,4] + 0.1*cf5 + 0.1*cf6))*7 + 13 + stats::rnorm(size,sd=4)
  }
  # else if (gps_spec==7){
  #   treat<- (-0.8+0.1*cf[,1]+0.1*cf[,2]- 0.1*cf[,3] + 0.2*cf[,4] + 0.1*cf5 + 0.1*cf6)*15 + 22 + rt(size,2) #+ rcauchy(size)
  # }
  #produce outcome Y
  Y = sapply(1:size, function(i){
