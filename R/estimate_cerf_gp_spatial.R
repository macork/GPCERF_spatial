...
#' }
#'
estimate_cerf_gp_spatial <- function(data, w, gps_m, params,
  ...
  if (nthread > 1 && nrow(tune_params_subset) > 1) {
    ...
    # Tune res needs to be defined in some way
    tune_res <- c()
  }
  ...
  gp_cerf_final <- compute_m_sigma_spatial(hyperparam = opt_param,
                                           outcome_data =  outcome_data,
                                           treatment_data = treatment_data,
                                           covariates_data = covariates_data,
                                           w = w,
