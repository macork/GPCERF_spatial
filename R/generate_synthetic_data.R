...
#'
#' @return
#' A data frame of the synthetic data. Outcome is labeled as Y, exposure as w, and
#' covariates cf1-6.
#'
#' @export
#'
...
generate_synthetic_data <-function(sample_size=1000, outcome_sd = 10,
  ...
  if (gps_spec==1){
    treat<-(-0.8+0.1*cf[,1]+0.1*cf[,2]- 0.1*cf[,3] + 0.2*cf[,4] + 0.1*cf5 + 0.1*cf6)*9 +
      17 + stats::rnorm(sample_size,sd=5)
  }else if (gps_spec==2){
    treat<-(-0.8+0.1*cf[,1]+0.1*cf[,2]- 0.1*cf[,3] + 0.2*cf[,4] + 0.1*cf5 + 0.1*cf6)*15 +
      22 + stats::rt(sample_size,2)
    treat[which(treat<(-5))]<-(-5)
    treat[which(treat>(25))]<-(25)
  }else if (gps_spec==3){
    treat <- (-0.8+0.1*cf[,1]+0.1*cf[,2]- 0.1*cf[,3] + 0.2*cf[,4] + 0.1*cf5 + 0.1*cf6)*9 +
      1.5*cf[,3]^2 + stats::rnorm(sample_size,mean=0,5) + 15
  }else if (gps_spec==4){
    treat <- 49*exp((-0.8+0.1*cf[,1]+0.1*cf[,2]- 0.1*cf[,3] + 0.2*cf[,4] + 0.1*cf5 + 0.1*cf6))/(1+exp((-0.8+0.1*cf[,1]+0.1*cf[,2]- 0.1*cf[,3] + 0.2*cf[,4] + 0.1*cf5 + 0.1*cf6)))-6 + stats::rnorm(sample_size,sd=5)
  }else if (gps_spec==5){
    treat <- 42/(1+exp((-0.8+0.1*cf[,1]+0.1*cf[,2]- 0.1*cf[,3] + 0.2*cf[,4] + 0.1*cf5 + 0.1*cf6))) -18 + stats::rnorm(sample_size,sd=5)
  }else if (gps_spec==6){
    treat <- log(abs(-0.8+0.1*cf[,1]+0.1*cf[,2]- 0.1*cf[,3] + 0.2*cf[,4] + 0.1*cf5 + 0.1*cf6))*7 + 13 + stats::rnorm(sample_size,sd=4)
  }
  # else if (gps_spec==7){
  #   treat<- (-0.8+0.1*cf[,1]+0.1*cf[,2]- 0.1*cf[,3] + 0.2*cf[,4] + 0.1*cf5 + 0.1*cf6)*15 + 22 + rt(sample_size,2) #+ rcauchy(sample_size)
  # }
  #produce outcome Y
  Y = sapply(1:sample_size, function(i){
    -10 - sum(c(2, 2, 3, -1)*cf[i,]) - 2*cf5[i] - 2*cf6[i] -
      treat[i]*(0.1 - 0.1*cf[i,1] + 0.1*cf[i,4] + 0.1*cf5[i] + 0.1*cf[i,3]^2) + 0.13^2*treat[i]^3 +
